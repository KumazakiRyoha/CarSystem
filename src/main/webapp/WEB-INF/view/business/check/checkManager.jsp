...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${morron}/check/loadAllCheck.action' //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "${morron}/check/loadAllCheck.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on("toolbar(checkTable)",function (obj) {
            ...
            if (layEvent==='deleteBatch'){
                ...
                layer.confirm('真的要删除这些检查单吗？',function (index) {
                    //向服务端发送删除指令
                    $.post("${morron}/check/deleteBatchCheck.action",params,function (res) {
                        layer.msg(res.msg);
                        tableIns.reload();
                    })
                })
        ...
        table.on('tool(checkTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.checkid + '】这个检查单么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${morron}/check/deleteCheck.action", {checkid: data.checkid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openUpdateCheck(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${morron}/check/updateCheck.action";
                }
            });
        }
